...
export default function RegisterRequest() {
  ...
  const [confirmation, setConfirmation] = useState({ show: false, action: null, userID: null });
  ...
  const handleAccept = (userID) => {
    setConfirmation({ show: true, action: 'accept', userID });
  };

  const handleReject = (userID) => {
    setConfirmation({ show: true, action: 'reject', userID });
  };

  const handleConfirm = async () => {
    const { action, userID } = confirmation;
    ...
    try {
      ...
      setConfirmation({ show: false, action: null, userID: null });
  ...
  const getInstitutionName = (instituteID) => {
    if (!instituteID) return "Sin institución";
    return `Institución ID: ${instituteID}`;
  };
        ...
        <div className="w-full max-w-5xl bg-white shadow-lg rounded-3xl p-4 md:p-10">
          ...
          {isLoading ? (
              ...
              solicitudes.map((solicitud, index) => (
                  <div key={solicitud.userID} className="border-b pb-4 md:pb-6 mb-4 md:mb-6">
                    <div
                        onClick={() => toggleRequest(index)}
                        className="flex items-center justify-between cursor-pointer"
                    >
                      <h2 className="text-base md:text-xl font-semibold text-gray-900 pr-2">
                        {`${solicitud.firstName} ${solicitud.fLastName} ${solicitud.sLastName || ''}`}
                      </h2>
                    ...
                    {selectedRequest === index && (
                        <div className="mt-4">
                          <div className="text-sm md:text-base space-y-2">
                            <p className="break-words"><strong>Correo:</strong> {solicitud.email}</p>
                            <p><strong>Teléfono:</strong> {solicitud.phone}</p>
                            <p><strong>Rol esperado:</strong> {solicitud.role}</p>
                            <p><strong>Institución:</strong> {getInstitutionName(solicitud.instituteID)}</p>
                          </div>

                          <div className="mt-4 flex flex-col sm:flex-row gap-3 md:gap-4">
                            <button
                                onClick={() => handleAccept(solicitud.userID)}
                            ...
                            <button
                                onClick={() => handleReject(solicitud.userID)}
